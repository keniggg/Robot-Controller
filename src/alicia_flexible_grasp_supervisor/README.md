...
2026.9.20：剩余额度搜索已在第二轮实机完成7步补偿，编码器FK残差17.874→5.903 mm，通过原6 mm/5°门；350帧原始回包中J5保持不变。后续第六轮被支撑面间距门拒绝，第七轮近场准备超时；七轮均未闭爪或提起。反馈采样与最终新帧交接修复曾加载，但相应实机分支尚未验收，跨姿态几何和候选耗时仍待解决。用户现已关闭硬件及WSL，后续只做离线处理；使能记录和节点PID仅描述历史实验。详见[最新实验与证据](../../docs/verification/2026-09-20-pregrasp-budget-guide.md)。

离线后续已减少重复接触测量和倾角CAD计算，并补齐计算完成时的期限检查及失败请求源帧记录；302项和930项回归分组通过（有重叠，另2项真实样本缺失跳过）。新增修改未部署，详见[离线修复与验证](../../docs/verification/2026-09-20-offline-followup.md)。
