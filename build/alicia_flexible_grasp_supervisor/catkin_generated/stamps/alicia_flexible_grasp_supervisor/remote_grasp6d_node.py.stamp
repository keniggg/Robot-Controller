...
class RemoteGrasp6DNode:
    ...
    def _process_latest_frame(self):
        ...
        try:
            ...
            if selected is None:
                self._publish_error('remote 6D returned %d candidates, none reachable' % len(candidates))
                return
            plan_msg = make_grasp_plan_pose_array(grasp_pose, stamp, rospy.get_param('/grasp', {}))
            self.plan_pub.publish(plan_msg)
            self.status_pub.publish(String('remote 6D plan ready score=%.3f width=%.3f' % (selected.score, selected.width_m)))
            self.last_error = ''
        except Exception as exc:
            self._publish_error('remote 6D planning failed: %s' % exc)
