...
## 01:54审批恢复后的1.5mm实机结果

审批恢复后已提交f0c3cd7并部署到独立remote服务，体素为1.5mm。
远场05a6faae7a99e11a880fb932于01:56:37接纳，复用19.21cm观察位；
近场阶段3于01:56:41.585开始，01:56:49.338以
NEAR_FIELD_NO_HARD_SAFE_CANDIDATE结束。未闭爪或抬升。
1510点融合表面配准1008 inliers、83.03%重叠、0.678mm RMSE，
局部桌面差0.388mm。17个桌面proposal中13个超出单侧伸达界限，
其余共同接触高度1.001–1.469mm，低于2mm；没有生成接触候选。
保存于根目录`.ros_log/grasp_attempt_20260910_fine_surface/`。

连续四个新RGB-D窗口的两对离线配准再次得到零候选，1.5mm融合
1469/1516点，进一步加密1mm得到3258/3358点仍为零。因此后续采用
有边界的观察视角恢复，不再仅靠点数推断接触可行性。原始数据及
回放位于`.ros_log/surface_resolution_20260910_repeat/`。

同时远场请求5在排队12.614秒后丢失了精确时间TF（默认历史仅10秒），
已用真实tf2 BufferCore复现。修复保留30秒TF历史，仍查快照精确时间，
不修改快照年龄、执行有效期或任何接触门。相关测试与恢复工作在
9月11日继续，见次日记录。
